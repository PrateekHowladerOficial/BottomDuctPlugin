...
namespace TeklaPH
{
    ...
    {
        public static Vector FindPerpendicularVectorInXYPlane(Vector inputVector)
        {
            // Ensure the input vector is valid
            if (inputVector == null)
            {
                throw new ArgumentNullException(nameof(inputVector), "Input vector cannot be null.");
            }

            // Handle edge case: If the input vector is a zero vector, return a zero vector
            if (inputVector.X == 0 && inputVector.Y == 0 && inputVector.Z == 0)
            {
                throw new ArgumentException("Input vector cannot be a zero vector.");
            }

            // Case 1: If the input vector is already in the XY plane, simply return a perpendicular vector in the XY plane
            if (inputVector.Z == 0)
            {
                return new Vector(-inputVector.Y, inputVector.X, 0); // Perpendicular in the XY plane
            }

            // Case 2: General case where the input vector is not in the XY plane
            // Select an arbitrary vector in the XY plane
            Vector xyPlaneVector = new Vector(1, 0, 0); // X-axis vector

            // Compute the cross product of the input vector with the XY plane vector
            double x = inputVector.Y * xyPlaneVector.Z - inputVector.Z * xyPlaneVector.Y;
            double y = inputVector.Z * xyPlaneVector.X - inputVector.X * xyPlaneVector.Z;
            double z = inputVector.X * xyPlaneVector.Y - inputVector.Y * xyPlaneVector.X;

            Vector perpendicular = new Vector(x, y, z);

            // Normalize the perpendicular vector
            double length = Math.Sqrt(perpendicular.X * perpendicular.X + perpendicular.Y * perpendicular.Y + perpendicular.Z * perpendicular.Z);
            perpendicular = new Vector(perpendicular.X / length, perpendicular.Y / length, perpendicular.Z / length);

            // Minimize the Z component by projecting onto the XY plane
            perpendicular = new Vector(perpendicular.X, perpendicular.Y, 0);
